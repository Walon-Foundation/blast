...
pub fn save(record: &HistoryRecord) -> Result<()> {
    ...
    let name = format!("{}_{}.json", record.timestamp, path_hash(&record.config_path));
...
pub fn load_last(config_path: &str) -> Option<HistoryRecord> {
    ...
    let mut records: Vec<HistoryRecord> = fs::read_dir(&dir).ok()?
        .filter_map(|e| e.ok())
...
pub fn diff_print(prev: &HistoryRecord, curr: &HistoryRecord) {
    ...
    for (label, pv, cv) in [
        ...
        let arrow = if pct > 5.0 { "▲" } else if pct < -5.0 { "▼" } else { "→" };
        println!("    {}  {}ms → {}ms ({:+.0}%) {}", label, pv as u128, cv as u128, pct, arrow);
    }
}
